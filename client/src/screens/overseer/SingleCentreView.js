...
class SingleCentreView extends Component {
	returnUniqueArray = a => {
		let arr = [];
	...
	getDataForDate = date => {
		// console.log(
		// 	this.props.navigation.getParam('data').attendance.filter(
	...
	returnDataLabels = () => {
		if (_.isEmpty(this.props.navigation.getParam('data').attendance)) {
			return ['0/0'];
		}
			...
			<ScrollView showsVerticalScrollIndicator={false}>
				...
				<Animatable.View animation='fadeInDown' duration={1000} delay={200}>
					...
					>
						...
						>
							{this.props.auth.user.center} Centre Full Data History
						</Text>
					</Card>
				</Animatable.View>
				...
				<Animatable.View animation='fadeInDown' duration={1500} delay={250}>
					<Card containerStyle={{ ...styles.card, height: 550, marginBottom: 20 }}>
				...
				{/* {console.log(this.getDates(this.props.navigation.getParam('data').attendance))} */}
			</ScrollView>
		);
	}
}

SingleCentreView['navigationOptions'] = screenProps => ({
	title: 'Data History',
});
...
export default connect(mapStateToProps, null)(SingleCentreView);
