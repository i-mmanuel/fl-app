...
class SontasDataHistory extends Component {
	returnUniqueArray = a => {
		let arr = [];
				...
				<Animatable.View animation='fadeInDown' duration={1500} delay={250}>
					<Card containerStyle={{ ...styles.card, height: 550, marginBottom: 20 }}>
						<View style={styles.row}>
							<Text style={{ marginVertical: 10, fontWeight: 'bold' }}>Names Of Centres</Text>
						</View>
						<FlatList
							...
							renderItem={({ item }) => {
								return (
									<TouchableOpacity
										onPress={() =>
											this.props.navigation.navigate('SontaView', {
									...
									>
										<View style={styles.row}>
											<Text style={{ color: 'black', marginVertical: 10 }}>{item.location}</Text>
...
SontasDataHistory['navigationOptions'] = screenProps => ({
...
export default connect(mapStateToProps, null)(SontasDataHistory);
