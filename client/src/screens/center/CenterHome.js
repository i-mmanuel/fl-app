	...
	componentDidMount() {
		...
		if (
			...
			this.timer = setInterval(() => {
				this.fetchData();
				console.log(`center fetched new data`);
			}, 30000);
		}
	}
			...
			<ScrollView showsVerticalScrollIndicator={false}>
				...
				<Animatable.View animation='fadeInDown' duration={1100} delay={150}>
					<Card containerStyle={{ ...styles.card, height: 100, marginTop: -50 }}>
						...
						>
							{this.isCenterLeader()
								? '• ' + this.props.auth.user.center + ' Centre •'
				...
				<Animatable.View animation='fadeInDown' duration={1200} delay={150}>
					<Card
						containerStyle={{
							...styles.card,
							height: this.isCenterLeader() ? 300 : 100,
						}}
					>
						{this.isCenterLeader() && (
							<TouchableOpacity
								...
								onPress={() =>
									this.props.navigation.navigate('BACENTA', { userID: this.props.navigation.getParam('userID') })
								}
							>
								<Text style={{ ...styles.buttonTitle }}>MY BACENTA</Text>
							</TouchableOpacity>
						)}
						...
						<TouchableOpacity
							...
							onPress={() =>
								this.props.navigation.navigate('AllLeaders', {
									center: this.checkLeader(),
								})
							}
						>
							<Text style={{ ...styles.buttonTitle }}>BACENTA LEADERS</Text>
						</TouchableOpacity>
					</Card>
				</Animatable.View>
				...
				{this.props.center.centerData && this.props.center.centerData.attendance.length >= 1 ? (
					<Animatable.View animation='fadeInDown' delay={150} duration={1500}>
						...
						>
							...
							>
								<Text style={{ ...styles.buttonTitle }}>VIEW CENTRE DATA</Text>
							</TouchableOpacity>
						</Card>
					</Animatable.View>
				) : (
					<Animatable.View animation='fadeInDown' delay={150} duration={1700}>
...
export default connect(mapStateToProps, { AddCenterData, AddLeadersData })(CenterHome);
