...
class DataHistory extends Component {
	...
	returnDataSets = () => {
		...
		if (dates.length >= 7) {
			return _.slice(dates, dates.length - 8).map(item =>
				this.sift(this.getDataForDate(item.date), 'attendance_number').reduce(this.reducer)
			);
		} else {
			return dates.map(item => this.sift(this.getDataForDate(item.date), 'attendance_number').reduce(this.reducer));
		}
	};
			...
			<ScrollView showsVerticalScrollIndicator={false}>
				...
				<Animatable.View animation='fadeInDown' duration={1000} delay={200}>
					...
					>
						...
						>
							{this.props.navigation.getParam('center')} Centre Full Data History
						</Text>
					</Card>
				</Animatable.View>
...
DataHistory['navigationOptions'] = screenProps => ({
	title: 'Data History',
});
...
export default connect(mapStateToProps, null)(DataHistory);
