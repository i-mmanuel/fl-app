...
class AllLeaderData extends Component {
				...
				<Animatable.View animation='fadeInDown' duration={1500} delay={230}>
					<Card containerStyle={{ ...styles.card, height: 350, marginBottom: 50 }}>
						...
						<FlatList
							...
							renderItem={({ item }) => {
								return (
									<TouchableOpacity onPress={() => this.props.navigation.navigate('LeaderDateView', { item })}>
										<View style={styles.row}>
...
AllLeaderData['navigationOptions'] = screenProps => ({
	title: 'All Leader Data',
});

export default connect(mapStateToProps, null)(AllLeaderData);
